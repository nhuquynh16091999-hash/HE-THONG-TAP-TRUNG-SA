...
type Verdict = "khop" | "lech_tien" | "chua_ve_tien" | "thua_o_sao_ke";
...
type StatementMeta = {
    ...
    detected_columns: string[]; missing_columns: string[];
};
...
const VERDICTS: { id: Verdict | "all"; label: string; hint: string }[] = [
    ...
    { id: "lech_tien", label: "Lệch tiền", hint: "3PL trả khác số POS ghi" },
    { id: "chua_ve_tien", label: "Chưa về tiền", hint: "đã giao nhưng sao kê chưa có" },
    { id: "thua_o_sao_ke", label: "Thừa ở sao kê", hint: "sao kê có mà POS không thấy đơn" },
...
const VERDICT_LABEL: Record<Verdict, string> = {
    khop: "Khớp", lech_tien: "Lệch tiền",
    chua_ve_tien: "Chưa về tiền", thua_o_sao_ke: "Thừa ở sao kê",
};
...
export default function TALPHACodReconTab({ dateRange }: Props) {
    ...
    const [filter, setFilter] = useState<Verdict | "all">("lech_tien");
    ...
    const exportCsv = () => {
        const head = ["Kết luận", "Mã đơn", "Vận đơn", "Ngày đơn", "Trạng thái POS",
            "Tiền POS (TWD)", "Tiền sao kê (TWD)", "Lệch", "Phí", "Ngày TT", "Marketer", "Sale"];
        const esc = (v: unknown) => `"${String(v ?? "").replace(/"/g, '""')}"`;
        const body = shown.map((l) => [
            VERDICT_LABEL[l.verdict], l.order_id, l.tracking, l.order_date || "", l.status_name || "",
            l.pos_amount, l.stm_amount, l.diff, l.fee, l.paid_date, l.marketer || "", l.sale || "",
        ].map(esc).join(","));
    ...
    return (
        <div className="space-y-5">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div>
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">
                            File CSV hoặc TSV. Excel thì mở ra rồi Lưu thành .csv trước.
                            Khớp theo <span className="font-mono">{matchKey === "tracking" ? "mã vận đơn" : "mã đơn"}</span>.
                        </p>
                    </div>
            ...
            {summary && (
                <>
                    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                        ...
                        <Stat label="Phí 3PL trên sao kê" value={TWD(summary.fee_total)}
                            sub={`POS ${TWD(summary.pos_total)} · sao kê ${TWD(summary.stm_total)}`} />
                    </div>
            ...
            <p className="text-xs text-muted-foreground">
                ...
                lý do trả tiền. “Chưa về tiền” là tiền thật đang nằm ở đơn vị vận chuyển, không phải lỗi dữ liệu.
            </p>
        </div>
    );
}
