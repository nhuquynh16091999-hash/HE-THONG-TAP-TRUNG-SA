...
| **Gán đơn → sản phẩm** (tab sản phẩm trong file từng người) | **Theo PAGE** (luồng cũ, chốt lại 16/09/2026): sản phẩm của đơn = sản phẩm đang chạy trên page của đơn. Mối nối page ↔ sản phẩm học từ chính đơn POS có cả `page_id` lẫn `ad_id`, cộng số page ghi trong tên camp nếu có. Page chạy nhiều sản phẩm → theo quảng cáo của chính đơn; đơn không ghi quảng cáo → sản phẩm nhiều đơn nhất của page; page chưa nối được → quảng cáo của đơn → `(khác)`. Người được tính đơn vẫn theo dòng trên (tag POS trước) — page chỉ quyết định TAB. Code: `san_pham_cua_don` | Không có tab sản phẩm |
