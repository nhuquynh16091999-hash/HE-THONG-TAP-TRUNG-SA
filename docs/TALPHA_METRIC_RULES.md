...
| **Gán đơn → marketer** | Rule CEO: 1) TAG trong POS (`marketer.$.name`); 2) không tag → chủ **campaign nối theo nguồn đơn** (dòng dưới), trượt thì chủ campaign của `ad_id`/`adset_id`; 3) vẫn không → tab **"(không gán)"**. Gán tay ở tab MAP TAY thắng tất cả. KHÔNG bỏ đơn lặng lẽ | `ad_id` → ad; không khớp thì fallback `marketer+market+page_id` (chỉ khi khoá trỏ đúng một campaign); vẫn không → "chưa map" kèm lý do |
| **Nối đơn → campaign** (sản phẩm, marketer, tiền ads) | **Theo NGUỒN ĐƠN** — Sỹ Anh chốt 17/09/2026: đơn POS → cột "Nguồn đơn" (tên page, đồng bộ vào `sale_order.page_name`) → khớp ô tên page trong tên camp Meta → camp đó. Khớp theo thứ tự: trùng tên → tên camp bắt đầu bằng tên page (ngày dính vào tên) → tên page nằm lệch ô. Page nhiều camp → quảng cáo của chính đơn → camp tiêu tiền trên page đúng ngày đơn về (thiếu thì ngày gần nhất trước đó) → camp tiêu nhiều nhất. Không nối được → theo `ad_id`, và đơn hiện ở tab **CHƯA MAP** của file TỔNG TEAM. Người gán tay ở tab **MAP TAY** (theo mã đơn hoặc cả page) — job không bao giờ ghi đè tab đó. Sale = `assigning_seller` trên POS (`sale_order.seller_name`). Code: `tim_camp_theo_page`, `doc_map_tay` | Chưa dùng |
