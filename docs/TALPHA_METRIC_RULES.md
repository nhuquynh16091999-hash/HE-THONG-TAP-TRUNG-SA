...
| **Doanh số** | `cod ÷ số chia của shop × tỷ giá`. Đài Loan: **số chia 1**, tỷ giá **800đ/TWD**. Singapore, UAE (sắp chạy): **chưa có** — doanh thu 0 cho tới khi khai, không đoán. Nguồn: `talpha_rules.json → markets` (`pos_money_divisor` + `rate_vnd`) | Cùng |
