...
| **Doanh số** | `cod ÷ số chia của shop × tỷ giá`. Đài Loan: **số chia 1**, tỷ giá **800đ/TWD**. Singapore: **số chia 100** (tạm, chưa đo trên đơn SGD thật), tỷ giá **20.000đ/SGD**. UAE: **chưa có tỷ giá** — doanh thu 0 cho tới khi khai, không đoán. Giá vốn = `products.*.cost_price_rmb × cost_rate_rmb_vnd` (3.860). Nguồn: `talpha_rules.json → markets` (`pos_money_divisor` + `rate_vnd`) | Cùng |
